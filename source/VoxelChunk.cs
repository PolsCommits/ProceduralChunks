...
public class VoxelChunk : MonoBehaviour
{
    ...
    public static int PerlinHeight(int atX, int atY, VoxelChunk chunk, int maxHeight)
    {
        return (int)Mathf.Min(Mathf.PerlinNoise((chunk.chunkX * chunk.width + atX) / 10.0f, (chunk.chunkY * chunk.depth + atY) / 10f) * maxHeight, maxHeight - 1);
    }

    public static int ImprovedPerlinHeight(int atX, int atY, VoxelChunk chunk, int maxHeight)
    {
        return (int)Mathf.Min(
            (
            Mathf.PerlinNoise((chunk.chunkX * chunk.width + atX) / 10.0f, (chunk.chunkY * chunk.depth + atY) / 10f) +
            Mathf.PerlinNoise((chunk.chunkX * chunk.width + atX) / 20.0f, (chunk.chunkY * chunk.depth + atY) / 20f)
            )
            * maxHeight,
            chunk.height - 1
            );
